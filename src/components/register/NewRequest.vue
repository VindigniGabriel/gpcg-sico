<template>
    <v-dialog v-model="dialogNewRequest" max-width="500px" persistent>
        ...
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="red darken-3" flat @click="close" small outline>Cancelar</v-btn>
            <v-btn color="primary" flat @click="save" small outline>Agregar</v-btn>
        </v-card-actions>
        </v-card>
    </v-dialog>
</template>
...
export default {
    ...
    computed: {
        ...mapState(['dialogNewRequest', 'date', 'user', 'clientData']),
        phoneErrors () {
            const errors = []
    ...
    methods: {
        ...mapMutations(['changeDialogNewRequest']),
        close () {
            this.$v.$reset()
            this.changeDialogNewRequest(false)
            this.requests = '',
            this.phone = 416,
            this.lineNew = false,
            this.typeLine = '',
        ...
        firebase.firestore()
            ...
            .onSnapshot(typeLines => {
                ...
                typeLines.forEach(typeLine => {
                    this.typeLineItems.push(typeLine.data().name)
                })
            })
